...
@section('button')
    <li class="nav-item mr-2">
        <a href="{{ url('/cities/create') }}" class="btn btn-primary nav-link" style="color: white">+ Tambah</a>
    </li>
    <li class="nav-item mr-2">
...
@section('isi')
    <div class="d-none d-md-block">
        <form action="{{ url('/cities') }}" class="mr-2 ml-2">
                ...
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 30%; background-color:rgb(243, 243, 243);" class="text-center">Provinsi</th>
                    ...
                    <tbody>
                        ...
                        @else
                            @foreach ($cities as $key => $city)
                                <tr>
                                    <td class="text-center" style="position: sticky; left: 0; background-color: rgb(235, 235, 235); z-index: 1; vertical-align: middle;">{{ ($cities->currentpage() - 1) * $cities->perpage() + $key + 1 }}.</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $city->name ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $city->province->name ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $city->createdBy->name ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $city->updatedBy->name ?? '-' }}</td>
                                    <td style="vertical-align: middle;">
                                        <div style="display: flex; gap: 5px;">
                                            <a href="{{ url('/cities/edit/'.$city->id) }}" class="btn btn-primary btn-sm" style="border-radius: 10px;" title="Edit city"><i class="fa fa-edit"></i></a>

                                            <form action="{{ url('/cities/delete/'.$city->id) }}" method="post">
                                                @method('delete')
                                                @csrf
                                                <button title="Delete city" class="border-0 btn btn-danger btn-sm" style="border-radius: 10px;" onClick="return confirm('Are You Sure')"><i class="fa fa-trash"></i></button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
