...
class TransactionController extends Controller
{
    ...
    public function reportOwnerTransactions()
    {
        ...
        $transaction_in_paid = Transaction::where('owner_id', auth()->user()->id)
        ->where('in_out', 'in')
        ->where('status', 'paid')
        ...
        ->selectRaw('SUM(total_amount - owner_fee) as total')
        ->value('total');

        $transaction_in_unpaid = Transaction::where('owner_id', auth()->user()->id)
        ->where('in_out', 'in')
        ->where('status', 'unpaid')
        ->when(!$start_date && !$start_date && $month, function ($query) use ($month) {
            $query->where('month', $month);
        ...
        $transaction_out = Transaction::where('owner_id', auth()->user()->id)
        ->where('in_out', 'out')
        ...
        $transaction_in_paid_all = Transaction::where('owner_id', auth()->user()->id)->where('in_out', 'in')->where('status', 'paid')->selectRaw('SUM(total_amount - owner_fee) as total')
        ->value('total');
        $transaction_out_all = Transaction::where('owner_id', auth()->user()->id)->where('in_out', 'out')->selectRaw('SUM(total_amount - owner_fee) as total')
        ->value('total');

        $balance = $transaction_in_paid_all - $transaction_out_all;
        ...
        foreach ($months as $num => $name) {
            $transaction_in_paid_array[] = Transaction::where('owner_id', auth()->user()->id)
            ->where('in_out', 'in')
            ->where('status', 'paid')
            ->where('month', $num)
            ->when(!$start_date && !$end_date && $year, function ($query) use ($year) {
            ...
            $transaction_in_unpaid_array[] = Transaction::where('owner_id', auth()->user()->id)
            ->where('in_out', 'in')
            ->where('status', 'unpaid')
            ->where('month', $num)
            ->when(!$start_date && !$end_date && $year, function ($query) use ($year) {
            ...
            $transaction_out_array[] = Transaction::where('owner_id', auth()->user()->id)
            ->where('in_out', 'out')
            ->where('month', $num)
            ->when(!$start_date && !$end_date && $year, function ($query) use ($year) {
                $query->where('year', $year);
        ...
        return view('transactions.reportOwnerTransactions', compact(
            'title',
            'transaction_in_paid',
